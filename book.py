...
class book(row_data):
    ...
    def apad_update_d(self):
        if len(self.ap[self.ad[self.row_data.id][0]]) > 1: #the removal id is not the only id
            self.ap[self.ad[self.row_data.id][0]].remove(self.row_data.id)
        else:
            self.ap.pop(self.ad[self.row_data.id][0]) # the removal id is the only id
    ...
    def blevel_update_d(self):
        if self.bd[self.row_data.id][0] in self.top and len(self.bp[self.bd[self.row_data.id][0]]) == 1: #remove the price from the list if only one id
            self.top.remove(self.bd[self.row_data.id][0])
            try:
                self.top.append(sorted(self.bp.keys(),reverse=True)[5]) #add a new price to list
            except:
                self.top.append(0)

    def alevel_update_d(self):
        if self.ad[self.row_data.id][0] in self.bottom and len(self.ap[self.ad[self.row_data.id][0]]) == 1: #remove the price from the list
            self.bottom.remove(self.ad[self.row_data.id][0])
            try:
                self.bottom.append(sorted(self.ap.keys(),reverse=False)[5]) #add a new price to list
            except:
